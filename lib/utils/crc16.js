/**
 * From the official FEIG reader documentation:
 * 
 * x16 + x12 + x5 + 1 = 0x8408
 * start value: 0xFFFF
 * 
 * This CRC checksum is identified as "CRC-16/MCRF4XX" by Amarok79 (https://github.com/Amarok79/InlayTester.Drivers.FeigReader)
 * http://reveng.sourceforge.net/crc-catalogue/all.html
 * 
 * Implementation below is the fast lookup table method by GitHub user wensonsmith
 * https://gist.github.com/wensonsmith/b8a1bc275e38e2e907c74b546f0138ba
 */


/** @type {array} Lookup table */
const crcTable = [
...
/**
 * Generate crc16 checksum, uses lookup table to speed up the process.
 *
...
function crc16(data) {
    ...
    for (let m = 0; m < data.length; m++) {
        crc = crcTable[(data[m] ^ crc) & 0xFF] ^ (crc >> 8 & 0xFF)
    }
...
/**
 * Add CRC bytes to an existing buffer, also prepends the frame length.
 *
 * @param {Buffer} buffer Input buffer
 * @return {Buffer} Output buffer
 */
function addCrcToBuffer (buffer) {
    //console.log("add crc to",buffer)
    const finalLength = buffer.length +2
    //const finalBuffer = Buffer.from([...Array.from(buffer)])
    const crc = crc16(buffer)
    const output = Buffer.alloc(finalLength)
    for (let i = 0; i < buffer.length; i++) {
        output[i] = buffer[i]
    }
    output[output.length - 2] = crc & 0xff
    output[output.length - 1] = crc >> 8
    
    return output
}

/**function addCrcToBuffer (buffer) {
    const finalLength = buffer.length + 6
    const finalBuffer = Buffer.from([finalLength, ...Array.from(buffer)])
    const crc = crc16(finalBuffer)
    const output = Buffer.alloc(finalLength)
    for (let i = 0; i < finalBuffer.length; i++) {
        output[i] = finalBuffer[i]
    }
    output[output.length - 2] = crc & 0xff
    output[output.length - 1] = crc >> 8
...
exports = module.exports = crc16

exports.addCrcToBuffer =  addCrcToBuffer